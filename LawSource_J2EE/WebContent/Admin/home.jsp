...
$(document).ready(function()
{
	$.getJSON("newuserdetails.jsp",function(json)
	{
		...
		if(json.length==0)
		{
			$('#tablebody').append('<tr><td colspan="5" class="new_alignment">No New Clients</td></tr>');
		}
		else
		{
			while(i<json.length)
		...
		$.getJSON("newadvocatedetails.jsp",function(json)
		{
			...
			$('#tablebody').append('<tr><th colspan="5" class="new_alignment">New Advocates</th></tr>');
			if(json.length==0)
			{
			...
			$.getJSON("blockdetails.jsp",function(json)
				{
					...
					$('#tablebody').append('<tr><th colspan="5" class="new_alignment">Blocked Persons</th></tr>');
					if(json.length==0)
					{
						$('#tablebody').append('<tr ><td colspan="5">NoBody Blocked</td></tr>');
					}
					else
					{
						while(i<json.length)

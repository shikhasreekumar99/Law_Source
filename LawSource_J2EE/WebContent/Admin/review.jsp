...
{
	$.getJSON("reviewdetails.jsp",function(json)
	{
		var i=0;
		...
		else
		{
			while(i<json.length)
			{
				$('#tablebody').append('<tr><td>'+json[i].name+'</td><td>'+json[i].l_email+'</td><td>'+json[i].feedback+'</td><td>'+json[i].rating+'</td><td>'+json[i].c_name+'</td><td>'+json[i].c_email+'</td></tr>');
				i++;
			}
		}	
	});
